...
- 스케줄표: 방송출연, 시상식, 음악축제, 콘서트, 팬미팅, 팬싸 등 **합법적으로 공개된 스케줄만** 다룸.
  - 달력, 게시판, 카드 형태 중 선택 조회 가능. 기본값은 달력.
    - 달력: 년/월 선택(기본값은 현재 날짜의 년/월) 월달력 표시. 스케줄이 있는 날에 마크(스케줄 종류 별 색깔 구분)하고, 해당 일자의 칸 클릭 시  확장돼서 어떤 스케줄인지 간략하게 표시(여러 스케줄이 있을 수 있으니 리스트 형태). 스케줄 클릭 시 모달창(페이지 흐려짐 효과 X) 뜨면서 상세 정보 조회 가능.
    - 게시판: 년/월 선택(기본값은 현재 날짜의 년/월) 1일부터 말일까지 하루에 한 칸, 스케줄이 있는 날에 스케줄 간략하게 표시. 클릭 시 모달창(페이지 흐려짐 효과 X) 뜨면서 상세 정보 조회 가능.
    - 카드: 년/월 선택(기본값은 현재 날짜의 년/월) 1일부터 말일까지 하루에 카드 하나, 스케줄이 있는 날에 스케줄 간략하게 표시. 클릭 시 모달창(페이지 흐려짐 효과 X) 뜨면서 상세 정보 조회 가능.
  - 크롤링, 수동 등록 모두 가능하게 함.
- 투표 일정: 여러 투표 관련 매체(벅스, 멜론, 엠넷플러스 등의 음원서비스나 팬앤스타, 빅크, 쿠궁 등 시상식 관련)에서 진행되는 투표
  - 기본적으로 현재 진행중인 투표가 상단에 표시
  - (다른 형태 추후 추가 고려) 가로가 1.2배정도 긴, 둥근 모서리, 테두리가 투명한 직사각형 형태 카드 형식으로 출력. 
  - 지난 투표 조회, 매체별 투표 조회 등도 지원.
  - 크롤링, 수동 등록 모두 가능하게 함.
...
- 커뮤니티 (공지, 질문, 자유, 건의게시판 등 기본적인 CRUD)
  - 공지글은 관리자(admin, manager)만 작성 가능, 건의글은 관리자(admin, manager)만 조회 가능
